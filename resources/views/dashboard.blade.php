...
<div class="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
    <article class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <header class="flex items-center justify-between text-xs font-semibold uppercase text-slate-400">
            Ocupación
            <span class="rounded-full bg-slate-100 px-2 py-1 text-[11px] text-slate-500">{{ $stats['total_mesas'] ?? 0 }} mesas</span>
        </header>
        <p class="mt-4 text-3xl font-semibold text-slate-900">{{ $stats['ocupacion'] ?? 0 }}%</p>
        <p class="text-sm text-slate-500">{{ ($stats['ocupadas'] ?? 0) + ($stats['en_cuenta'] ?? 0) }} mesas en uso</p>
        <div class="mt-4 h-2 rounded-full bg-slate-100">
            <div class="h-full rounded-full bg-emerald-500" style="width: {{ $stats['ocupacion'] ?? 0 }}%"></div>
        </div>
    </article>
    <article class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <header class="text-xs font-semibold uppercase text-slate-400">Mesas libres</header>
        <p class="mt-4 text-3xl font-semibold text-slate-900">{{ $stats['libres'] ?? 0 }}</p>
    ...
    <article class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <p class="mt-4 text-3xl font-semibold text-slate-900">{{ $stats['pedidos_activos'] ?? 0 }}</p>
            ...
            @forelse($mesas as $mesa)
                ...
                <article class="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
                    ...
                    <div class="mt-3 space-y-2 text-xs text-slate-500">
                        @forelse($mesa->pedidos->whereIn('estado', ['pendiente', 'listo']) as $pedido)
