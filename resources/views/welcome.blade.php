...
<!DOCTYPE html>
    ...
    <head>
        ...
        <script>
            // Tailwind config centraliza la paleta y las sombras usadas en todo el landing
            tailwind.config = {
                theme: {
                    extend: {
                        fontFamily: {
                        ...
                        boxShadow: {
                            glass: '0 25px 60px -20px rgba(15, 23, 42, 0.45)',
                        },
                    },
                },
            };
    ...
    <body class="min-h-screen bg-gradient-to-br from-amber-50 via-orange-50 to-rose-50 font-sans text-slate-700 antialiased">
        <div class="relative overflow-hidden">
            <div class="pointer-events-none absolute inset-0 -z-10">
                <div class="absolute -left-20 top-10 h-64 w-64 rounded-full bg-brand-500 opacity-20 blur-3xl"></div>
                <div class="absolute right-0 top-32 h-72 w-72 rounded-full bg-rose-400 opacity-20 blur-3xl"></div>
            </div>

            <div class="relative mx-auto flex min-h-screen max-w-6xl flex-col px-6 pb-16 pt-10">
                <header class="flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between">
                    <div class="flex items-center gap-3">
                        <span class="flex h-12 w-12 items-center justify-center rounded-xl bg-white/70 text-2xl font-semibold text-brand-600 shadow-glass">🍽️</span>
                        <div>
                            <p class="text-sm uppercase tracking-wide text-brand-600">Restaurante POS</p>
                            <h1 class="font-display text-xl font-semibold text-slate-900">Gestiona sala, cocina y caja desde un panel unificado</h1>
                        </div>
                    </div>
                    ...
                    @if (Route::has('login'))
                        <nav class="flex items-center gap-3 text-sm">
                            @auth
                                <a href="{{ url('/dashboard') }}" class="rounded-full bg-slate-900 px-4 py-2 font-medium text-white shadow-lg shadow-slate-900/20 transition hover:bg-slate-700">Ir al panel</a>
                            @else
                                <a href="{{ route('login') }}" class="rounded-full border border-slate-300/60 bg-white/70 px-4 py-2 font-medium text-slate-700 shadow-sm transition hover:border-brand-500 hover:text-brand-600">Iniciar sesión</a>
                                @if (Route::has('register'))
                                    <a href="{{ route('register') }}" class="rounded-full bg-brand-500 px-4 py-2 font-medium text-white shadow-lg shadow-brand-500/30 transition hover:bg-brand-600">Crear cuenta</a>
                                @endif
                            @endauth
                        </nav>
                    @endif
                ...
                <main class="mt-16 grid flex-1 gap-12 lg:grid-cols-[1.1fr_0.9fr]">
                    <section class="flex flex-col justify-center space-y-10">
                        <div class="space-y-5">
                            <p class="inline-flex items-center gap-2 rounded-full border border-brand-100 bg-white/70 px-3 py-1 text-xs font-semibold uppercase tracking-[0.2em] text-brand-500 shadow-sm">Solución integral</p>
                            <h2 class="font-display text-4xl font-bold text-slate-900 sm:text-5xl">Una experiencia moderna para tu salón, cocina y punto de venta</h2>
                            <p class="text-lg text-slate-600">Controla mesas, pedidos y ventas con métricas en vivo y flujos claros. Tailwind nos permite mantener una interfaz ligera y enfocada en el servicio.</p>
                        </div>

                        <div class="grid gap-4 sm:grid-cols-2">
                            <div class="rounded-2xl bg-white/80 p-6 shadow-glass backdrop-blur">
                                <p class="text-sm font-semibold text-brand-500">Mesas en vivo</p>
                                <p class="mt-2 text-2xl font-bold text-slate-900">Disponibilidad al instante</p>
                                <p class="mt-3 text-sm text-slate-500">Revisa ocupación y tiempos de servicio sin abandonar el tablero central.</p>
                            </div>
                            <div class="rounded-2xl bg-white/80 p-6 shadow-glass backdrop-blur">
                                <p class="text-sm font-semibold text-brand-500">Pedidos a cocina</p>
                                <p class="mt-2 text-2xl font-bold text-slate-900">Flujo ordenado</p>
                                <p class="mt-3 text-sm text-slate-500">Envía, prioriza y completa platos con indicadores claros para el equipo.</p>
                            </div>
                            <div class="rounded-2xl bg-white/80 p-6 shadow-glass backdrop-blur">
                                <p class="text-sm font-semibold text-brand-500">Control de caja</p>
                                <p class="mt-2 text-2xl font-bold text-slate-900">Ventas resumidas</p>
                                <p class="mt-3 text-sm text-slate-500">Consulta ticket promedio y medios de pago sin reportes manuales.</p>
                            </div>
                            <div class="rounded-2xl bg-white/80 p-6 shadow-glass backdrop-blur">
                                <p class="text-sm font-semibold text-brand-500">Gestión de carta</p>
                                <p class="mt-2 text-2xl font-bold text-slate-900">Actualizaciones ágiles</p>
                                <p class="mt-3 text-sm text-slate-500">Activa, pausa o ajusta platos y visualiza su impacto por categoría.</p>
                            </div>
                        </div>

                        <div class="flex flex-wrap items-center gap-4 pt-4">
                            <a href="{{ route('login') }}" class="rounded-full bg-slate-900 px-6 py-3 text-base font-semibold text-white shadow-lg shadow-slate-900/20 transition hover:bg-slate-700">Entrar al sistema</a>
                            <p class="text-sm text-slate-500">¿Primera vez? Usa los accesos de prueba y recorre todo el flujo.</p>
                        </div>
                    </section>

                    <section class="relative">
                        <div class="absolute inset-0 rounded-3xl bg-white/70 shadow-glass backdrop-blur"></div>
                        <div class="relative flex h-full flex-col justify-between rounded-3xl border border-white/60 px-8 py-10">
                            <div class="space-y-6">
                                <div>
                                    <p class="text-sm font-semibold uppercase tracking-[0.3em] text-slate-400">Accesos de demostración</p>
                                    <h3 class="mt-2 font-display text-2xl font-semibold text-slate-900">Usuarios listos para probar</h3>
                                    <p class="mt-2 text-sm text-slate-500">Inicia sesión con cualquiera de estos perfiles para validar cada módulo.</p>
                                </div>

                                <div class="space-y-3 rounded-2xl bg-slate-900 p-6 text-slate-100 shadow-lg">
                                    <div class="flex items-center justify-between text-sm">
                                        <span class="font-semibold">Administrador</span>
                                        <code class="rounded bg-white/10 px-2 py-1">admin@restaurante.test / admin123</code>
                                    </div>
                                    <div class="flex items-center justify-between text-sm">
                                        <span class="font-semibold">Mozo</span>
                                        <code class="rounded bg-white/10 px-2 py-1">mozo@restaurante.test / mozo123</code>
                                    </div>
                                    <div class="flex items-center justify-between text-sm">
                                        <span class="font-semibold">Cocina</span>
                                        <code class="rounded bg-white/10 px-2 py-1">cocina@restaurante.test / cocina123</code>
                                    </div>
                                    <div class="flex items-center justify-between text-sm">
                                        <span class="font-semibold">Caja</span>
                                        <code class="rounded bg-white/10 px-2 py-1">caja@restaurante.test / caja123</code>
                                    </div>
                                </div>
                            </div>

                            <div class="mt-10 space-y-4">
                                <h4 class="text-sm font-semibold uppercase tracking-[0.3em] text-slate-400">Módulos disponibles</h4>
                                <ul class="grid gap-3 text-sm text-slate-600">
                                    <li class="flex items-center gap-2"><span class="text-brand-500">●</span> Dashboard con KPIs diarios y semanales</li>
                                    <li class="flex items-center gap-2"><span class="text-brand-500">●</span> Gestión de mesas con estado claro por mesa</li>
                                    <li class="flex items-center gap-2"><span class="text-brand-500">●</span> Cocina con control de tiempos y progreso de platos</li>
                                    <li class="flex items-center gap-2"><span class="text-brand-500">●</span> Catálogo de productos con filtros por categoría</li>
                                    <li class="flex items-center gap-2"><span class="text-brand-500">●</span> Historial de ventas con resumen por medio de pago</li>
                                </ul>
                            </div>
                        </div>
                    </section>
                ...
                <footer class="mt-16 flex flex-col gap-6 border-t border-white/60 pt-6 text-sm text-slate-500 sm:flex-row sm:items-center sm:justify-between">
                    <p>Desarrollado con Laravel {{ Illuminate\Foundation\Application::VERSION }} y Tailwind CSS.</p>
                    <div class="flex flex-wrap items-center gap-4">
                        <span class="flex items-center gap-2 text-xs uppercase tracking-widest text-slate-400"><span class="h-2 w-2 rounded-full bg-brand-500"></span> Proyecto demo listo para producción</span>
                        <a href="{{ route('login') }}" class="text-sm font-semibold text-brand-600 transition hover:text-brand-500">Acceder ahora →</a>
                    </div>
                </footer>
            </div>
        </div>
